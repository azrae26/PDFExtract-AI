        ...
        <div className="flex gap-2 items-end">
          <div className="flex-1 min-w-0">
            ...
            <div className="relative">
              ...
              >
                {(modelChoices ?? GEMINI_MODELS.map(m => ({ id: m.id, label: m.label, thinking: /pro/i.test(m.id) }))).map((m) => {
                  let label = m.label;
                  if ('priceInput' in m && m.priceInput != null) label += ` ($${m.priceInput}/$${(m as ModelChoice).priceOutput})`;
                  if (m.thinking) label += ' \u{1F9E0}';
          ...
          {onRefreshModels && (
            <div className="flex-shrink-0">
              <button
                ...
                className="w-[34px] h-[34px] flex items-center justify-center border border-gray-300 rounded-lg bg-gray-50 text-gray-500 hover:bg-gray-100 hover:text-indigo-600 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                title="重新探測可用模型"
              >
                <svg className={`w-4 h-4${isRefreshingModels ? ' animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
